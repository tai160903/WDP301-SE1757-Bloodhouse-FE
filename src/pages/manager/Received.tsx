...
export default function BloodRequests() {
  ...
  const mapApiDataToTableRequest = (req: any, index: number, page: number): TableRequest => ({
    ...
    receivedDate: req.createdAt ? new Date(req.createdAt).toLocaleDateString("vi-VN") : "N/A",
    ...
    preferredDate: req.preferredDate ? new Date(req.preferredDate).toLocaleDateString("vi-VN") : "N/A",
    medicalDocs: req.medicalDocumentUrl && Array.isArray(req.medicalDocumentUrl) ? req.medicalDocumentUrl : [],
  });
  ...
  const fetchRequests = useCallback(async (page: number) => {
    ...
    try {
      setLoading(true);
      setProgress(30);
      const response = await getBloodRequests(facilityId, page, pageSize, "createdAt", "-1");
      setProgress(60);
      console.log("Fetch Requests Response:", response);
      const requestsArray = Array.isArray(response) ? response : Array.isArray(response.data) ? response.data : [];
  ...
  const getUrgencyColor = (urgency: string) => {
    return urgency === "Khẩn cấp" ? "bg-red-100 text-red-800" : "bg-blue-100 text-blue-800";
  };
  ...
  // Xử lý phản hồi
  const handleResponse = useCallback((request: TableRequest, type: "accept" | "reject") => {
    ...
    <TooltipProvider>
      <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {[
            { title: "Tổng Yêu Cầu", value: summaryData.totalRequests, color: "blue" },
            { title: "Chờ Xem Xét", value: summaryData.pendingReview, color: "yellow" },
            { title: "Khẩn Cấp", value: summaryData.emergencyRequests, color: "red" },
            { title: "Đã Hoàn Thành", value: summaryData.fulfilledRequests, color: "green" },
          ].map((item) => (
            <Card key={item.title} className={`border-l-4 border-${item.color}-500 hover:shadow-lg transition-shadow`}>
        ...
        <Card className="shadow-lg">
          ...
          <CardContent>
              ...
              <TableBody>
                {filteredRequests.map((request) => (
                  <TableRow key={request.requestId} className="hover:bg-gray-50">
                    ...
                    <TableCell>{request.groupId?.name || request.bloodType}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end space-x-2">
                        ...
                        {request.status === BLOOD_REQUEST_STATUS.PENDING_APPROVAL && (
                          <>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
            ...
            <div className="flex justify-between items-center mt-6">
              ...
              <span className="text-gray-600">Trang {currentPage} / {totalPages}</span>
              <Button
                variant="outline"
        ...
        <Dialog open={isResponseDialogOpen} onOpenChange={setIsResponseDialogOpen}>
          <DialogContent>
            ...
            {responseType === "reject" && (
              <div className="mt-4">
                <Label htmlFor="responseNotes" className="text-sm font-medium text-gray-600">
                  Lý do từ chối
                </Label>
                <Textarea
                  id="responseNotes"
